...
contract Ownable {
    ...
    modifier onlyOwner(){
        require(msg.sender == _owner,"Only owner can invoke this function");
        _;
    }
...
contract Pausable is Ownable  {
    ...
    event Paused(address triggedBy);
    event Unpaused(address triggedBy);
    ...
    function getPaused() public onlyOwner returns (bool pauseStatus){
        return _paused;
    }
    function setPaused(bool pauseStatus) public onlyOwner{
        _paused = pauseStatus;
        if(_paused){
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        // TODO require the given address to not be the owner of the tokenId
            require(to!= _tokenOwner[tokenId],"To address is same as owner");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
           require((msg.sender == getOwner()) || isApprovedForAll(getOwner(),msg.sender) ,"Only the owner of contract can call or approvalforall should be true");
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(_isApprovedOrOwner(msg.sender, tokenId),"Approval needed ");
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        ...
        bool res = tokenOwnerAddress != address(0);
        return res;
    }
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal  returns (bool) {
        ...
        return ( spender == getOwner() || spender == ownerOfToken || getApproved(tokenId) == spender || isApprovedForAll(ownerOfToken, spender));
    }
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(ownerOf(tokenId) == from, "Only owner can tranfer the token");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function getSymbol() external returns(string memory symbol){
        return _symbol;
    }
    function getBaseTokenURI() external returns(string memory baseTokenURI) {
